...
页面顶部“成交方式”可选择“当天收盘”或“次日开盘（集合竞价模拟）”，切换后立即重算。当天收盘模式假设按最终收盘信号和同日收盘价成交，属于理想化回测；次日模式按下一交易日开盘价模拟成交，不保证实际集合竞价成交。没有下一交易日开盘数据时不计入已完成交易。日期筛选按实际买卖日期，年度交易收益按卖出年份统计；逐笔资金及交易 CSV 同步使用选定方式。此选项不改变后台通知时间和信号规则。
